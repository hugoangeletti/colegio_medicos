    ...
    if ($resSeguro['estado']) {
        ...
        if (isset($seguro) && sizeof($seguro) > 0) {
            if ($seguro['origen'] == 'COLEGIO') {
                ...
                $buscarArchivos = $caminoArchivos.$matricula.'*';
                ...
                if (file_exists($nombreArchivoCompleto)) {
                    //$nombreArchivoEnviar = utf8_decode($seguro['nombreArchivoCompleto']);
                    $nombreArchivoEnviarArray = explode('/', $nombreArchivoCompleto);
                    $nombreArchivoEnviar = $nombreArchivoEnviarArray[8];
                    $existe = TRUE;
                }  
                if ($existe) {
                    //echo '<br>'.$nombreArchivoEnviar;
                    //fin verificar pdf
