...
//firmantes UBA
$resFirmante = $colegiadoEspecialistaLogic->firmantesPorEntidad('UBA', 1);
if ($resFirmante['estado']) {
    $firmante = $resFirmante['datos'];
...
$resFirmante = $colegiadoEspecialistaLogic->firmantesPorEntidad('UBA', 2);
if ($resFirmante['estado']) {
    $firmante = $resFirmante['datos'];
...
if ($continua) {
    ...
    //parrafo 2
    $html = 'Por lo tanto, la Facultad de Medicina de la Universidad de Buenos Aires y el Colegio de Médicos de la Provincia de Buenos Aires, le otorgan la Certificación del Título de Especialista en: ';
    $y = 133;
    $pdf->writeHTMLCell(320, 0, $x, $y, $html, 0, 1, 0, true, 'J', true);
    ...
    $laFecha = 'Buenos Aires, '.substr($fechaAprobada, 8, 2).' de '.obtenerMes(substr($fechaAprobada, 5, 2)).' de '.substr($fechaAprobada, 0, 4);
