...
if (isset($_GET['agregar'])) {
    ...
    if (isset($_POST['fechaEntrega']) && $_POST['fechaEntrega'] <> "") {
        ...
        $continua = FALSE;
        $mensaje .= "Falta fechaEntrega - ";
    }
    if (isset($_POST['idTipoEntrega']) && $_POST['idTipoEntrega'] <> "") {
        $idTipoEntrega = $_POST['idTipoEntrega'];
        if ($idTipoEntrega == 3) {
            //si es entrega de titulo especialista
            if (isset($_POST['idTituloEspecialista']) && $_POST['idTituloEspecialista'] <> "") {
                $idTituloEspecialista = $_POST['idTituloEspecialista'];
            } else {
                $continua = FALSE;
                $mensaje .= "Falta idTituloEspecialista - ";
            }
        } else {
    ...
    switch ($accion) {
        case "AGREGAR":
            ...
            $datosTipoMesaEntrada = array(
                                    ...
                                    'idTituloEspecialista' => $idTituloEspecialista
                                );
            ...
            //$resultado = $mesaEntradaLogic->agregarMesaEntradaEntrega($idColegiado, $idTipoMesaEntrada, $observaciones, $estadoMatricular, $estadoTesoreria, $idTipoEntrega, $fechaEntrega);
            break;

        case "BORRAR":
            //borrar       
            $resultado = $mesaEntradaLogic->borrarMesaEntradaEntrega($idMesaEntrada);
            break;
...
<body onLoad="document.forms['myForm'].submit()">
    ...
    ?>
        <form name="myForm"  method="POST" action="../mesa_entrada_entrega.php">
            <input type="hidden"  name="mensaje" id="mensaje" value="<?php echo $resultado['mensaje']; ?>">
            <input type="hidden"  name="clase" id="clase" value="<?php echo $resultado['clase'];?>">
            ...
            <input type="hidden"  name="fechaEntrega" id="fechaEntrega" value="<?php echo $fechaEntrega;?>">
            <input type="hidden"  name="idTipoEntrega" id="idTipoEntrega" value="<?php echo $idTipoEntrega;?>">
