...
if (isset($idMesaEntrada) && $idMesaEntrada > 1) {
    //guarda pdf
    //var_dump($mesaEntradaMovimientoAnulado); exit;
    ...
    $pdf->SetFont('dejavusans', 'B', 8);
    $pdf->MultiCell(0, 5, 'Movimiento Anulado: ', 0, 'L', false, 0, '', '');
    $pdf->SetFont('dejavusans', '', 8);
    if ($idTipoMovimiento == 'F' || $idTipoMovimiento == 'J') {
        $pdf->MultiCell(0, 5, $nombreTipoMovimiento, 0, 'L', false, 1, '90', '');
    } else {
        $pdf->MultiCell(0, 5, $nombreTipoMovimiento.' ('.$nombreTipoMovimientoCompleto.')', 0, 'L', false, 1, '90', '');
    }
