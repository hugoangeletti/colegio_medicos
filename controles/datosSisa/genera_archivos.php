...
if (isset($_GET['id']) && $_GET['id'] <> "") {
	...
	if ($sisaLogic->siExisteExportacion($periodoProceso)) {
		$continua = FALSE;
		$mensaje .= "YA EXISTE LA EXPORTACION DEL MES.";
	}
}

if ($continua) {
	if (!isset($idSisaExportacion)) {
		$nombreArchivoColegiados = 'ColegiadosDistritoI_'.$periodoProceso.'.txt';
		$nombreArchivoEspecialistas = 'EspecialistasDistritoI_'.$periodoProceso.'.txt';
		$path = "../../archivos/sisa/".date('Y')."/".date('m');
		$resArchivo = $sisaLogic->agregarSisaExportacion($periodoProceso, $nombreArchivoColegiados, $nombreArchivoEspecialistas, $path);
		if ($resArchivo['estado']) {
			$idSisaExportacion = $resArchivo['idSisaExportacion'];
		} else {
	...
	if ($continua) {
		...
		if (file_exists($path."/".$nombreArchivoColegiados)) {
		    unlink($path."/".$nombreArchivoColegiados);
		}
		if (file_exists($path."/".$nombreArchivoEspecialistas)) {
		    unlink($path."/".$nombreArchivoEspecialistas);
		}

		$fileColegiados = fopen($path."/".$nombreArchivoColegiados, "w")or  die("Problemas en la creacion del archivo ".$path."/".$nombreArchivoColegiados);
		$fileEspecialistas = fopen($path."/".$nombreArchivoEspecialistas, "w")or  die("Problemas en la creacion del archivo ".$path."/".$nombreArchivoEspecialistas);
