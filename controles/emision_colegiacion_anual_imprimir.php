    ...
    if (isset($_POST['colegiados'])) {
        ...
        if (isset($_GET['idColegiado'])) {
            ...
            if (isset($_GET['emisionTotal'])) {
                ...
                $tipoPdf = 'F';
                $envioMail = FALSE;
                $resColegiados = $colegiadoDeudaAnualLogic->obtenerColegiadosEmisionAnualTotal(PERIODO_ACTUAL);
    ...
    foreach ($colegiados as $colegiadoImprimir) {
        ...
        if ($resColegiado['estado'] && $resColegiado['datos']) {
            ...
            */
            ...
            if ($resDeuda['estado']) {
                ...
                foreach ($resDeuda['datos'] as $dato) {
                    ...
                    if ($estado == "A") {
                        ...
                        //para el periodo 2024/2025 a pedido de la Mesa Directiva solo se muestran por trimestre las chequeras, 24/6/2024
                        $vencimientoTrimestre = NULL;
                        if (CHEQUERA_TRIMESTRAL && !$esAgremicion) {
                        ...
                        $resCuotas = $colegiadoDeudaAnualLogic->obtenerDeudaAnualCuotas($idColegiadoDeudaAnual, $vencimientoTrimestre);
                        if ($resCuotas['estado']) {
                            ...
                            foreach ($resCuotas['datos'] as $cuotas) {
                                ...
                                if ($estado == 1) {
                                    ...
                                    */
                                    ...
                                    //$pdf->SetFont('dejavusans', 'B', 9);
                                    //$pdf->MultiCell(0, 5, $idColegiadoDeudaAnualCuota, 0, 'L', false, 1, '', '', true);
                                    $pdf->SetXY(90, $posLinea-25);
                                    $pdf->SetFont('dejavusans', 'B', 9);
                                    //$pdf->MultiCell(0, 5, 'Período: '.$periodo, 0, 'L', false, 1, '', '', true);
                                    $pdf->MultiCell(0, 5, 'Período: '.$periodo.' (Julio-'.$periodo.' a Junio-'.($periodo+1).')', 0, 'L', false, 1, '', '', true);
                                    $pdf->SetXY(170, $posLinea-25);
                                    $pdf->SetFont('dejavusans', 'B', 14);
                                    $pdf->MultiCell(0, 5, 'CUOTA '.$cuota, 0, 'L', false, 1, '', '', true);
                                    $codigoBarra = $colegiadoDeudaAnualLogic->obtenerCodigoBarra44($idColegiadoDeudaAnualCuota, $importeActualizado, $importeActualizado, $fechaVencimiento, $fechaVencimiento, NULL);
                            ...
                            //para el periodo 2024/2025 a pedido de la Mesa Directiva no se imprime el pago total, 24/6/2024
                            $vencimientoTrimestre = NULL;
                            //if (PERIODO_ACTUAL <> 2024 || $esAgremicion) {
                            //if (!CHEQUERA_TRIMESTRAL || $esAgremicion) {
                            if (TRUE) {
                                $generoChequera = TRUE;
                                ...
                                */
                                ...
                                if ($resPagoTotal['estado']) {
                                    ...
                                    if ($fechaVencimiento >= date('Y-m-d') && $importeActualizado > 0) {
                                        ...
                                        $pdf->MultiCell(100, 5, 'Pago Total Anual con descuento: $'.number_format($importeActualizado, 2, ',', ''), 0, 'L', false, 1, '', '', true);
                                        $pdf->SetXY(80, $posLinea-20);
                                    $pdf->SetFont('dejavusans', 'B', 9);
                                    //$pdf->MultiCell(0, 5, 'Período: '.$periodo, 0, 'L', false, 1, '', '', true);
                                    $pdf->MultiCell(0, 5, 'Período: '.$periodo.' (Julio-'.$periodo.' a Junio-'.($periodo+1).') - Vencimiento: '.cambiarFechaFormatoParaMostrar($fechaVencimiento), 0, 'L', false, 1, '', '', true);
            ...
            if (isset($_GET['emisionTotal']) && $generoChequera) {
                $resColegiado = $colegiadoLogic->obtenerColegiadoPorId($idColegiado);
                if ($resColegiado['estado'] && $resColegiado['datos']) {
                    $colegiado = $resColegiado['datos'];
                    $matricula = $colegiado['matricula'];
