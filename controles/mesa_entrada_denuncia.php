...
if (isset($_GET['id']) && $_GET['id'] <> "") {
    ...
    $idMesaEntradaDenuncia = $_GET['id'];
    $idMesaEntrada = NULL;
    $resDenuncia = $mesaEntradaLogic->obtenerMesaEntradaDenunciaPorId($idMesaEntradaDenuncia, $idMesaEntrada);
    if ($resAutoprescripcion['estado']) {
        $denuncia = $resDenuncia['datos'];
        $idMesaEntrada = $denuncia['idMesaEntrada'];
...
$titulo = "DENUNCIA DE ROBO/EXTRAVÍO";
...
<div class="panel panel-default">
    ...
    <div class="panel-body">
        ...
        if ($continua) {
            ?>  
            <form id="formJota" name="formJota" method="POST" onSubmit="" action="datosMesaEntrada\abm_denuncia.php?<?php if ($accion == "AGREGAR") { echo 'agregar'; } else { echo 'editar'; } ?>">
                <div class="row">
                    <div class="col-md-2">
                ...
                <div class="row">
                    <div class="col-md-3">
                        ...
                        <select class="form-control" id="idTipoDenuncia" name="idTipoDenuncia" required="" <?php echo $readOnly; ?>>
                            <option value="">Seleccione Tipo</option>
                            <?php 
                            $idTipoDenuncia = NULL;
                            $resTiposDenuncia = $mesaEntradaLogic->obtenerTiposDenuncia();
                            if ($resTiposDenuncia['estado']) {
                                foreach ($resTiposDenuncia['datos'] as $dato) {
                                ?>
                                    <option value="<?php echo $dato['id']; ?>" <?php if ($idTipoDenuncia == $dato['id']) { echo 'selected'; } ?>><?php echo $dato['nombre']; ?></option>
                                <?php
                                }
                            }
                            ?>
                    ...
                    <div class="col-md-3">
                        <label for="fechaDenuncia">Fecha de denuncia: </label>
                        <input class="form-control" type="date" name="fechaDenuncia" id="fechaDenuncia" <?php echo $readOnly; ?>>
                    </div>
                    <div class="col-md-3">
                        <label for="fechaExtravio">Fecha de Extravío / Falsificación / Robo: </label>
                        <input class="form-control" type="date" name="fechaExtravio" id="fechaExtravio" <?php echo $readOnly; ?>>
                    </div>
                </div>
                <div class="row">&nbsp;</div>
                <div class="row">
