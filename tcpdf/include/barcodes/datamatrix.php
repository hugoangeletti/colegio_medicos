...
// -----------------------------------------------------------------------------
	...
	protected function lookAheadTest($data, $pos, $mode) {
		...
		while (true) {
			...
			// get char
			$chr = ord((string)$data[$pos + $charscount]);
			$charscount++;
			// STEP L
		...
		while ($pos < $data_length) {
			...
			switch ($enc) {
				case ENC_ASCII: { // STEP B. While in ASCII encodation
					if (($data_length > 1) AND ($pos < ($data_length - 1)) AND ($this->isCharMode(ord((string)$data[$pos]), ENC_ASCII_NUM) AND $this->isCharMode(ord((string)$data[$pos + 1]), ENC_ASCII_NUM))) {
						// 1. If the next data sequence is at least 2 consecutive digits, encode the next two digits as a double digit in ASCII mode.
						$cw[] = (intval(substr((string)$data, $pos, 2)) + 130);
						++$cw_num;
						$pos += 2;
					} else {
