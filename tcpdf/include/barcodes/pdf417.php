	...
	protected function getInputSequences($code) {
		...
		foreach($numseq[1] as $seq) {
			...
			if ($seq[1] > 0) {
				...
				$prevseq = substr((string)$code, $offset, ($seq[1] - $offset));
				$textseq = array();
				// get text sequences
				...
				foreach($textseq[1] as $txtseq) {
					...
					if ($txtseq[1] > 0) {
						...
						$prevtxtseq = substr((string)$prevseq, $txtoffset, ($txtseq[1] - $txtoffset));
		...
		switch($mode) {
					...
					if (($k = array_search($chval, $this->textsubmodes[$submode])) !== false) {
						...
						for ($s = 0; $s < 4; ++$s) {
							...
							if (($s != $submode) AND (($k = array_search($chval, $this->textsubmodes[$s])) !== false)) {
								...
								if (((($i + 1) == $codelen) OR ((($i + 1) < $codelen) AND (array_search(ord((string)$code[($i + 1)]), $this->textsubmodes[$submode]) !== false))) AND (($s == 3) OR (($s == 0) AND ($submode == 1)))) {
			...
			case 924: { // Byte Compaction mode latch
				while (($codelen = strlen((string)$code)) > 0) {
					...
					if ($sublen == 6) {
						$t = bcmul(''.ord((string)$code[0]), '1099511627776');
						$t = bcadd($t, bcmul(''.ord((string)$code[1]), '4294967296'));
						$t = bcadd($t, bcmul(''.ord((string)$code[2]), '16777216'));
						$t = bcadd($t, bcmul(''.ord((string)$code[3]), '65536'));
						$t = bcadd($t, bcmul(''.ord((string)$code[4]), '256'));
						$t = bcadd($t, ''.ord((string)$code[5]));
