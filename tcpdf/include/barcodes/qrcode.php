...
class QRcode {
	...
	protected function binarize($frame) {
		...
		foreach ($frame as &$frameLine) {
			for ($i=0; $i<$len; $i++) {
				$frameLine[$i] = (ord((string)$frameLine[$i])&1)?'1':'0';
			}
		}
		return $frame;
	}
	...
	protected function setFrameAt($at, $val) {
		$this->frame[$at['y']][$at['x']] = chr((string)$val);
	}
	...
	protected function getFrameAt($at) {
		return ord((string)$this->frame[$at['y']][$at['x']]);
	}
	...
	protected function getNextPosition() {
		do {
			...
			$this->x = $x;
			$this->y = $y;
		} while(ord((string)$this->frame[$y][$x]) & 0x80);
	 ...
	 */
	 protected function writeFormatInformation($width, &$frame, $mask, $level) {
		...
		for ($i=0; $i<8; ++$i) {
			...
			$frame[8][$width - 1 - $i] = chr((string)$v);
			if ($i < 6) {
				$frame[$i][8] = chr((string)$v);
			} else {
				$frame[$i + 1][8] = chr((string)$v);
			}
			$format = $format >> 1;
		}
		for ($i=0; $i<7; ++$i) {
		...
		$frame[$width - 7 + $i][8] = chr((string)$v);
		if ($i == 0) {
			$frame[8][7] = chr((string)$v);
		} else {
			$frame[8][6 - $i] = chr((string)$v);
		}
		$format = $format >> 1;
		}
		return $blacks;
	 ...
	 */
	 protected function makeMaskNo($maskNo, $width, $s, &$d, $maskGenOnly=false) {
		...
		for ($y=0; $y<$width; ++$y) {
			for ($x=0; $x<$width; ++$x) {
				if ($bitMask[$y][$x] == 1) {
					$d[$y][$x] = chr(ord((string)$s[$y][$x]) ^ ((int)($bitMask[$y][$x])));
				}
				$b += (int)(ord((string)$d[$y][$x]) & 1);
			}
		}
		return $b;
	}
	 ...
	 */
	 protected function evaluateSymbol($width, $frame) {
		...
		for ($y=0; $y<$width; ++$y) {
			...
			for ($x=0; $x<$width; ++$x) {
				if (($x > 0) AND ($y > 0)) {
					$b22 = ord((string)$frameY[$x]) & ord((string)$frameY[$x-1]) & ord((string)$frameYM[$x]) & ord((string)$frameYM[$x-1]);
					$w22 = ord((string)$frameY[$x]) | ord((string)$frameY[$x-1]) | ord((string)$frameYM[$x]) | ord((string)$frameYM[$x-1]);
					if (($b22 | ($w22 ^ 1)) & 1) {
						$demerit += N2;
					}
				}
				if (($x == 0) AND (ord((string)$frameY[$x]) & 1)) {
					$this->runLength[0] = -1;
					$head = 1;
					$this->runLength[$head] = 1;
				} elseif ($x > 0) {
					if ((ord((string)$frameY[$x]) ^ ord((string)$frameY[$x-1])) & 1) {
						$head++;
						$this->runLength[$head] = 1;
					} else {
						$this->runLength[$head]++;
		...
		for ($x=0; $x<$width; ++$x) {
			...
			$this->runLength[0] = 1;
			for ($y=0; $y<$width; ++$y) {
				if (($y == 0) AND (ord((string)$frame[$y][$x]) & 1)) {
					$this->runLength[0] = -1;
					$head = 1;
					$this->runLength[$head] = 1;
				} elseif ($y > 0) {
					if ((ord((string)$frame[$y][$x]) ^ ord((string)$frame[$y-1][$x])) & 1) {
						$head++;
						$this->runLength[$head] = 1;
					} else {
						$this->runLength[$head]++;
	 ...
	 */
	 protected function isdigitat($str, $pos) {
		...
		return ((ord((string)$str[$pos]) >= ord('0'))&&(ord((string)$str[$pos]) <= ord('9')));
	}
	 ...
	 */
	 protected function isalnumat($str, $pos) {
		...
		return ($this->lookAnTable(ord((string)$str[$pos])) >= 0);
	}
	 ...
	 protected function identifyMode($pos) {
		...
		if ($this->isdigitat($this->dataStr, $pos)) {
			...
			if ($pos+1 < strlen((string)$this->dataStr)) {
				...
				$word = (ord((string)$c) << 8) | ord((string)$d);
	 ...
	 protected function toUpper() {
		...
		while ($p < $stringLen) {
			...
			if ($mode == QR_MODE_KJ) {
				...
				if ((ord((string)$this->dataStr[$p]) >= ord('a')) AND (ord((string)$this->dataStr[$p]) <= ord('z'))) {
					$this->dataStr[$p] = chr(ord((string)$this->dataStr[$p]) - 32);
				}
				$p++;
			}
		}
	 ...
	 */
	 protected function encodeModeNum($inputitem, $version) {
		...
		for ($i=0; $i < $words; ++$i) {
			$val  = (ord((string)$inputitem['data'][$i*3  ]) - ord('0')) * 100;
			$val += (ord((string)$inputitem['data'][$i*3+1]) - ord('0')) * 10;
			$val += (ord((string)$inputitem['data'][$i*3+2]) - ord('0'));
			$inputitem['bstream'] = $this->appendNum($inputitem['bstream'], 10, $val);
		}
		if ($inputitem['size'] - $words * 3 == 1) {
			$val = ord((string)$inputitem['data'][$words*3]) - ord('0');
			$inputitem['bstream'] = $this->appendNum($inputitem['bstream'], 4, $val);
		} elseif (($inputitem['size'] - ($words * 3)) == 2) {
			$val  = (ord((string)$inputitem['data'][$words*3  ]) - ord('0')) * 10;
			$val += (ord((string)$inputitem['data'][$words*3+1]) - ord('0'));
	 ...
	 */
	 protected function encodeModeAn($inputitem, $version) {
		...
		for ($i=0; $i < $words; ++$i) {
			$val  = (int)($this->lookAnTable(ord((string)$inputitem['data'][$i*2])) * 45);
			$val += (int)($this->lookAnTable(ord((string)$inputitem['data'][($i*2)+1])));
			$inputitem['bstream'] = $this->appendNum($inputitem['bstream'], 11, $val);
		}
		if ($inputitem['size'] & 1) {
			$val = $this->lookAnTable(ord((string)$inputitem['data'][($words * 2)]));
	 ...
	 */
	 protected function encodeMode8($inputitem, $version) {
		...
		for ($i=0; $i < $inputitem['size']; ++$i) {
			$inputitem['bstream'] = $this->appendNum($inputitem['bstream'], 8, ord((string)$inputitem['data'][$i]));
		}
		return $inputitem;
	}
	 ...
	 */
	 protected function encodeModeKanji($inputitem, $version) {
		...
		for ($i=0; $i<$inputitem['size']; $i+=2) {
			$val = (ord((string)$inputitem['data'][$i]) << 8) | ord((string)$inputitem['data'][$i+1]);
			if ($val <= 0x9ffc) {
				$val -= 0x8140;
			} else {
				$val -= 0xc140;
	 ...
	 */
	 protected function encodeModeStructure($inputitem) {
		...
		$inputitem['bstream'] = $this->appendNum($inputitem['bstream'], 4, ord((string)$inputitem['data'][1]) - 1);
		$inputitem['bstream'] = $this->appendNum($inputitem['bstream'], 4, ord((string)$inputitem['data'][0]) - 1);
		$inputitem['bstream'] = $this->appendNum($inputitem['bstream'], 8, ord((string)$inputitem['data'][2]));
		return $inputitem;
	}
	 ...
	 */
	 protected function checkModeNum($size, $data) {
		for ($i=0; $i<$size; ++$i) {
			if ((ord((string)$data[$i]) < ord('0')) OR (ord((string)$data[$i]) > ord('9'))){
				return false;
			}
		}
		return true;
	 ...
	 */
	 protected function checkModeAn($size, $data) {
		for ($i=0; $i<$size; ++$i) {
			if ($this->lookAnTable(ord((string)$data[$i])) == -1) {
				return false;
			}
		}
		return true;
	 ...
	 */
	 protected function checkModeKanji($size, $data) {
		...
		for ($i=0; $i<$size; $i+=2) {
			$val = (ord((string)$data[$i]) << 8) | ord((string)$data[$i+1]);
