...
class TCPDF_COLORS {
	...
	public static function convertHTMLColorToDec($hcolor, &$spotc, $defcol=array('R'=>128,'G'=>128,'B'=>128)) {
		...
		} elseif ((substr((string)$color, 0, 4) != 'cmyk') AND (substr((string)$color, 0, 3) != 'rgb') AND (($dotpos = strpos((string)$color, '.')) !== false)) {
			// remove class parent (i.e.: color.red)
			$color = substr((string)$color, ($dotpos + 1));
		...
		// RGBA ARRAY
		if (substr((string)$color, 0, 4) == 'rgba') {
			$codes = substr((string)$color, 5);
		...
		// RGB ARRAY
		if (substr((string)$color, 0, 3) == 'rgb') {
			$codes = substr((string)$color, 4);
			$codes = str_replace(')', '', $codes);
			$returncolor = explode(',', $codes);
			foreach ($returncolor as $key => $val) {
				if (strpos((string)$val, '%') > 0) {
		...
		// CMYK ARRAY
		if (substr((string)$color, 0, 4) == 'cmyk') {
			$codes = substr((string)$color, 5);
			$codes = str_replace(')', '', $codes);
			$returncolor = explode(',', $codes);
			foreach ($returncolor as $key => $val) {
		...
		if ($color[0] != '#') {
			...
			$color_code = substr((string)$color, 1);
		}
	...
	public static function _JScolor($color) {
		if (substr((string)$color, 0, 1) == '#') {
			return sprintf("['RGB',%F,%F,%F]", (hexdec(substr((string)$color, 1, 2)) / 255), (hexdec(substr((string)$color, 3, 2)) / 255), (hexdec(substr((string)$color, 5, 2)) / 255));
		}
