...
class TCPDF_FONTS {
	...
	public static function addTTFfont($fontfile, $fonttype='', $enc='', $flags=32, $outpath='', $platid=3, $encid=1, $addcbbox=false, $link=false) {
		...
		if (!isset($font_path_parts['filename'])) {
			$font_path_parts['filename'] = substr((string)$font_path_parts['basename'], 0, -(strlen((string)$font_path_parts['extension']) + 1));
		}
		...
		$replace = array('b', 'i', 'i', '');
		$font_name = str_replace((string)$search, $replace, $font_name);
		if (empty($font_name)) {
			// set generic name
			$font_name = 'tcpdffont';
		}
		...
		if (empty($fonttype)) {
			...
			} elseif (substr((string)$font, 0, 4) == 'OTTO') {
		...
		if ($fmetric['type'] == 'Type1') {
			// ---------- TYPE 1 ----------
			// read first segment
			$a = unpack('Cmarker/Ctype/Vsize', substr((string)$font, 0, 6));
			if ($a['marker'] != 128) {
				// Font file is not a valid binary Type1
				return false;
			}
			$fmetric['size1'] = $a['size'];
			$data = substr((string)$font, 6, $fmetric['size1']);
			// read second segment
			$a = unpack('Cmarker/Ctype/Vsize', substr((string)$font, (6 + $fmetric['size1']), 6));
			if ($a['marker'] != 128) {
				// Font file is not a valid binary Type1
				return false;
			}
			$fmetric['size2'] = $a['size'];
			$encrypted = substr((string)$font, (12 + $fmetric['size1']), $fmetric['size2']);
			$data .= $encrypted;
			...
			$rawbvl = explode(' ', trim((string)$matches[1]));
			...
			$eplain = '';
			for ($i = 0; $i < $elen; ++$i) {
				$chr = ord((string)$encrypted[$i]);
				$eplain .= chr((string)$chr ^ ($r >> 8));
				$r = ((($chr + $r) * $c1 + $c2) % 65536);
			}
			...
			$fmetric['Leading'] = 0;
			// get charstring data
			$eplain = substr((string)$eplain, (strpos((string)$eplain, '/CharStrings') + 1));
			...
			foreach ($matches as $k => $v) {
				...
				while ($i < $clen) {
					if ($ccom[$i] < 32) {
						...
						$sval = chr((string)$ccom[($i + 1)]).chr((string)$ccom[($i + 2)]).chr((string)$ccom[($i + 3)]).chr((string)$ccom[($i + 4)]);
			...
			for ($i = 0; $i < $numTables; ++$i) {
				// get table info
				$tag = substr((string)$font, $offset, 4);
				$offset += 4;
				$table[$tag] = array();
			...
			for ($i = 0; $i < $numNameRecords; ++$i) {
				...
				if ($nameID == 6) {
					...
					$fmetric['name'] = substr((string)$font, $offset, $stringLength);
		...
		if (!empty($fmetric['cbbox'])) {
			$pfile .= '$cbbox=array('.substr((string)$fmetric['cbbox'], 1).');'."\n";
		}
		$pfile .= '$cw=array('.substr((string)$fmetric['cw'], 1).');'."\n";
		$pfile .= '// --- EOF ---'."\n";
		// store file
	...
	public static function _getTTFtableChecksum($table, $length) {
		...
		$offset = 0;
		for ($i = 0; $i < $tlen; ++$i) {
			$v = unpack('Ni', substr((string)$table, $offset, 4));
			$sum += $v['i'];
			$offset += 4;
		}
	...
	public static function _getTrueTypeFontSubset($font, $subsetchars) {
		...
		for ($i = 0; $i < $tot_num_glyphs; ++$i) {
			if (isset($subsetglyphs[$i])) {
				...
				$glyf .= substr((string)$font, ($glyf_offset + $indexToLoc[$i]), $length);
			} else {
				$length = 0;
			}
			if ($short_offset) {
		...
		foreach ($table as $tag => $val) {
			if (in_array($tag, $table_names)) {
				$table[$tag]['data'] = substr((string)$font, $table[$tag]['offset'], $table[$tag]['length']);
				if ($tag == 'head') {
					// set the checkSumAdjustment to 0
					$table[$tag]['data'] = substr((string)$table[$tag]['data'], 0, 8)."\x0\x0\x0\x0".substr((string)$table[$tag]['data'], 12);
				}
		...
		$font = substr((string)$font, 0, $table['head']['offset'] + $offset + 4).pack('N', $checkSumAdjustment).substr((string)$font, $table['head']['offset'] + $offset + 8);
		return $font;
	}
	...
	public static function updateCIDtoGIDmap($map, $cid, $gid) {
		if (($cid >= 0) AND ($cid <= 0xFFFF) AND ($gid >= 0)) {
			...
			$map[($cid * 2)] = chr((string)$gid >> 8);
			$map[(($cid * 2) + 1)] = chr((string)$gid & 0xFF);
		}
		return $map;
	}
...
// ====================================================================================================================
	...
	public static function arrUTF8ToUTF16BE($unicode, $setbom=false) {
		...
		foreach ($unicode as $char) {
			if ($char == 0x200b) {
				...
				$outstr .= chr((string)$char >> 0x08);
				$outstr .= chr((string)$char & 0xFF);
			} else {
				$char -= 0x10000;
				$w1 = 0xD800 | ($char >> 0x0a);
				$w2 = 0xDC00 | ($char & 0x3FF);
				$outstr .= chr((string)$w1 >> 0x08);
				$outstr .= chr((string)$w1 & 0xFF);
				$outstr .= chr((string)$w2 >> 0x08);
				$outstr .= chr((string)$w2 & 0xFF);
			}
		}
		return $outstr;
	}
	...
	public static function getUniord($uch) {
		...
		for ($i = 0; $i < $length; ++$i) {
			$char = ord((string)$uch[$i]); // get one string character at time
