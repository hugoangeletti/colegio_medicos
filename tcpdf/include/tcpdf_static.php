...
class TCPDF_STATIC {
	...
	* @since 5.9.121 (2011-09-28)
		...
		foreach ($replace as $rep) {
			foreach ($rep[3] as $a) {
				if (strpos((string)$page, $a) !== false) {
					$page = str_replace((string)$a, $rep[0], $page);
					$diff += ($rep[2] - $rep[1]);
				}
			}
		}
	...
	public static function _AES($key, $text) {
		...
		$text .= str_repeat(chr((string)$padding), $padding);
		if (extension_loaded('openssl')) {
			...
			return $iv.substr((string)$text, 0, -16);
		}
	...
	public static function _AESnopad($key, $text) {
		if (extension_loaded('openssl')) {
			...
			$text = openssl_encrypt($text, $algo, $key, OPENSSL_RAW_DATA, $iv);
			return substr((string)$text, 0, -16);
		}
	...
	public static function _RC4($key, $text, &$last_enc_key, &$last_enc_key_c) {
		...
		if ($last_enc_key != $key) {
			$k = str_repeat((string)$key, (int) ((256 / strlen((string)$key)) + 1));
			$rc4 = range(0, 255);
			$j = 0;
			for ($i = 0; $i < 256; ++$i) {
				$t = $rc4[$i];
				$j = ($j + $t + ord((string)$k[$i])) % 256;
				$rc4[$i] = $rc4[$j];
				$rc4[$j] = $t;
			}
			$last_enc_key = $key;
	...
	public static function convertStringToHexString($s) {
		...
		foreach ($chars as $c) {
			$bs .= sprintf('%02s', dechex(ord((string)$c)));
		}
		return $bs;
	}
	...
	public static function getEncPermissionsString($protection) {
		...
		$str = chr(bindec(substr((string)$binprot, 24, 8)));
		$str .= chr(bindec(substr((string)$binprot, 16, 8)));
		$str .= chr(bindec(substr((string)$binprot, 8, 8)));
		$str .= chr(bindec(substr((string)$binprot, 0, 8)));
		return $str;
	}
	...
	public static function extractCSSproperties($cssdata) {
		...
		if (substr((string)$cssdata, -1) == '}') {
			// remove last parethesis
			$cssdata = substr((string)$cssdata, 0, -1);
		}
		...
		foreach ($cssblocks as $key => $block) {
			if (strpos((string)$block[0], ',') > 0) {
				...
				foreach ($selectors as $sel) {
					$cssblocks[] = array(0 => trim((string)$sel), 1 => $block[1]);
				}
				unset($cssblocks[$key]);
			}
		}
		...
		if (preg_match_all('/([\>\+\~\s]{1})([a-zA-Z0-9\*]+)([^\>\+\~\s]*)/si', $selector, $matches, PREG_PATTERN_ORDER | PREG_OFFSET_CAPTURE) > 0) {
			...
			if (($lasttag == '*') OR ($lasttag == $tag)) {
				...
				if (!empty($attrib)) {
					...
					switch ($attrib[0]) {
						case '.': { // class
							if (in_array(substr((string)$attrib, 1), $class)) {
								$valid = true;
							}
							break;
						}
						case '#': { // ID
							if (substr((string)$attrib, 1) == $id) {
								$valid = true;
							}
							break;
						}
							...
							if (preg_match('/\[([a-zA-Z0-9]*)[\s]*([\~\^\$\*\|\=]*)[\s]*["]?([^"\]]*)["]?\]/i', $attrib, $attrmatch) > 0) {
								...
								if (isset($dom[$key]['attribute'][$att])) {
									switch ($attrmatch[2]) {
										...
										case '^=': {
											if ($val == substr((string)$dom[$key]['attribute'][$att], 0, strlen((string)$val))) {
												$valid = true;
											}
											break;
										}
										case '$=': {
											if ($val == substr((string)$dom[$key]['attribute'][$att], -strlen((string)$val))) {
												$valid = true;
											}
											break;
										}
	...
	public static function getCSSdataArray($dom, $key, $css) {
		...
		foreach($css as $selector => $style) {
			...
			// get specificity
			$specificity = substr((string)$selector, 0, $pos);
			// remove specificity
			$selector = substr((string)$selector, $pos);
	...
	public static function getHyphenPatternsFromTEX($file) {
		...
		$data = trim(substr((string)$matches[0], 10, -1));
	...
	public static function pregSplit($pattern, $modifiers, $subject, $limit=NULL, $flags=NULL) {
		...
		if ((strpos((string)$modifiers, 'u') === FALSE) OR (count(preg_split('//u', "\n\t", -1, PREG_SPLIT_NO_EMPTY)) == 2)) {
			$ret = preg_split((string)$pattern.$modifiers, $subject, $limit, $flags);
			if ($ret === false) {
				return array();
			}
		...
		while (($nl = strpos((string)$subject, "\n")) !== FALSE) {
			$ret = array_merge($ret, preg_split((string)$pattern.$modifiers, substr((string)$subject, 0, $nl), $limit, $flags));
			$ret[] = "\n";
			$subject = substr((string)$subject, ($nl + 1));
		}
		if (strlen((string)$subject) > 0) {
			$ret = array_merge($ret, preg_split((string)$pattern.$modifiers, $subject, $limit, $flags));
		}
		return $ret;
	}
	...
	public static function fileGetContents($file) {
		...
		if ((strlen((string)$file) > 1)
		    ...
		    $findroot = strpos((string)$file, $_SERVER['DOCUMENT_ROOT']);
		...
		if (preg_match('%^(https?)://%', $url)
			...
			if (empty($urldata['query'])) {
				...
				if (strpos((string)$url, $host) === 0) {
				    // convert URL to full server path
				    $tmp = str_replace((string)$host, $_SERVER['DOCUMENT_ROOT'], $url);
	...
	public static function _getULONG($str, $offset) {
		$v = unpack('Ni', substr((string)$str, $offset, 4));
		return $v['i'];
	}
	...
	public static function _getUSHORT($str, $offset) {
		$v = unpack('ni', substr((string)$str, $offset, 2));
		return $v['i'];
	}
	...
	public static function _getSHORT($str, $offset) {
		$v = unpack('si', substr((string)$str, $offset, 2));
		return $v['i'];
	}
	...
	public static function _getBYTE($str, $offset) {
		$v = unpack('Ci', substr((string)$str, $offset, 1));
		return $v['i'];
	}
	/**
