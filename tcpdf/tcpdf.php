...
class TCPDF {
	...
	public function AddFont($family, $style='', $fontfile='', $subset='default') {
		...
		$tempstyle = strtoupper((string)$style === null ? '' : $style);
		$style = '';
		// underline
		...
		if (TCPDF_STATIC::empty_string($fontfile) OR (!@TCPDF_STATIC::file_exists($fontfile))) {
			...
			$tmp_fontfile = str_replace(' ', '', $family).strtolower((string)$style).'.php';
	...
	public function setLink($link, $y=0, $page=-1) {
		$fixed = false;
		if (!empty($page) AND (substr((string)$page, 0, 1) == '*')) {
	...
	public function getNumLines($txt, $w=0, $reseth=false, $autopadding=true, $cellpadding=null, $border=0) {
		...
		for ($i = 0; $i < $length; ++$i) {
			...
			if (($c != 160)
					AND (($c == 173)
						OR preg_match((string)$this->re_spaces, TCPDF_FONTS::unichr($c, $this->isunicode))
						OR (($c == 45)
	...
	public function Image($file, $x=null, $y=null, $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false, $alt=false, $altimgs=array()) {
		...
		// normalize $dpi: empty string was silently coerced to 0 in PHP 7, throws TypeError in PHP 8
	...
	public function Output($name='doc.pdf', $dest='I') {
		...
		if ($this->sign) {
			...
			// remove last newline
			$pdfdoc = substr((string)$pdfdoc, 0, -1);
			// remove filler space
			...
			$pdfdoc = substr((string)$pdfdoc, 0, $byte_range[1]).substr((string)$pdfdoc, $byte_range[2]);
			...
			// extract signature
			$signature = substr((string)$signature, $pdfdoc_length);
			$signature = substr((string)$signature, (strpos((string)$signature, "%%EOF\n\n------") + 13));
			$tmparr = explode("\n\n", $signature);
			$signature = $tmparr[1];
			// decode signature
			$signature = base64_decode(trim((string)$signature));
			...
			$this->buffer = substr((string)$pdfdoc, 0, $byte_range[1]).'<'.$signature.'>'.substr((string)$pdfdoc, $byte_range[1]);
		...
		if ($destroyall AND !$preserve_objcopy && isset($this->file_id)) {
			...
			if (isset($this->imagekeys)) {
				foreach($this->imagekeys as $file) {
					if ((strpos((string)$file,  K_PATH_CACHE.'__tcpdf_'.$this->file_id.'_') === 0)
		...
		foreach ($aliases as $type => $alias) {
			foreach ($alias as $a) {
				...
				if (($pos = strpos((string)$page, $a)) !== false) {
					...
					$page = str_replace((string)$aa, $shift, $page);
				}
			}
		}
		return $page;
	...
	protected function _putpages() {
		...
		for ($n = 1; $n <= $num_pages; ++$n) {
			...
			// replace EPS marker
			$temppage = str_replace((string)$this->epsmarker, '', $temppage);
			//Page
						...
						case 'link': {
							if (is_string($pl['txt']) && !empty($pl['txt'])) {
								if ($pl['txt'][0] == '#') {
									...
									$annots .= ' /A <</S /GoTo /D /'.TCPDF_STATIC::encodeNameObject(substr((string)$pl['txt'], 1)).'>>';
								} elseif ($pl['txt'][0] == '%') {
									// embedded PDF file
									$filename = basename(substr((string)$pl['txt'], 1));
									...
									if (empty($parsedUrl['scheme']) AND (!empty($parsedUrl['path']) && strtolower(substr((string)$parsedUrl['path'], -4)) == '.pdf')) {
		...
		foreach ($this->FontFiles as $file => $info) {
			...
			if (!TCPDF_STATIC::empty_string($fontfile)) {
				...
				if ((!$compressed) AND (isset($info['length2']))) {
					$header = (ord((string)$font[0]) == 128);
					if ($header) {
						// strip first binary header
						$font = substr((string)$font, 6);
					}
					if ($header AND (ord((string)$font[$info['length1']]) == 128)) {
						// strip second binary header
						$font = substr((string)$font, 0, $info['length1']).substr((string)$font, ($info['length1'] + 6));
					}
	...
	protected function _putxobjects() {
		foreach ($this->xobjects as $key => $data) {
			if (isset($data['outdata'])) {
				$stream = str_replace((string)$this->epsmarker, '', trim((string)$data['outdata']));
	...
	protected function _putviewerpreferences() {
		...
		if (isset($vp['PrintPageRange'])) {
			...
			$out .= ' /PrintPageRange ['.substr((string)$PrintPageRangeNum,1).']';
		}
	...
	protected function _out($s) {
		if ($this->state == 2) {
			if ($this->inxobj) {
				...
				$pagebuff = $this->getPageBuffer($this->page);
				$page = substr((string)$pagebuff, 0, -$this->footerlen[$this->page]);
				$footer = substr((string)$pagebuff, -$this->footerlen[$this->page]);
	...
	public function addHtmlLink($url, $name, $fill=false, $firstline=false, $color=null, $style=-1, $firstblock=false) {
		if (isset($url[1]) AND ($url[0] == '#') AND is_numeric($url[1])) {
			...
			$lnkdata = explode(',', $url);
			if (isset($lnkdata[0]) ) {
				$page = substr((string)$lnkdata[0], 1);
	...
	protected function _objectkey($n) {
		...
		$objkey = substr((string)$objkey, 0, 16);
		return $objkey;
	}
	...
	protected function _Uvalue() {
		if ($this->encryptdata['mode'] == 0) { // RC4-40
			...
			for ($i = 1; $i <= 19; ++$i) {
				$ek = '';
				for ($j = 0; $j < $len; ++$j) {
					$ek .= chr(ord((string)$this->encryptdata['key'][$j]) ^ $i);
				}
				$enc = TCPDF_STATIC::_RC4($ek, $enc, $this->last_enc_key, $this->last_enc_key_c);
			}
			$enc .= str_repeat("\x00", 16);
			return substr((string)$enc, 0, 32);
		...
		if ($this->encryptdata['mode'] < 3) { // RC4-40, RC4-128, AES-128
			...
			if ($this->encryptdata['mode'] > 0) {
				...
				for ($i = 1; $i <= 19; ++$i) {
					$ek = '';
					for ($j = 0; $j < $len; ++$j) {
						$ek .= chr(ord((string)$owner_key[$j]) ^ $i);
					}
					$enc = TCPDF_STATIC::_RC4($ek, $enc, $this->last_enc_key, $this->last_enc_key_c);
				}
			}
	...
	protected function _fixAES256Password($password) {
		...
		return substr((string)$psw, 0, 127);
	}
	...
	protected function _generateencryptionkey() {
		...
		if (!$this->encryptdata['pubkey']) { // standard mode
			if ($this->encryptdata['mode'] == 3) { // AES-256
				...
				// Pad passwords
				$this->encryptdata['user_password'] = substr((string)$this->encryptdata['user_password'].TCPDF_STATIC::$enc_padding, 0, 32);
				$this->encryptdata['owner_password'] = substr((string)$this->encryptdata['owner_password'].TCPDF_STATIC::$enc_padding, 0, 32);
				// Compute O value
				$this->encryptdata['O'] = $this->_Ovalue();
			...
			foreach ($this->encryptdata['pubkeys'] as $pubkey) {
				...
				// extract signature
				$signature = substr((string)$signature, strpos((string)$signature, 'Content-Disposition'));
				$tmparr = explode("\n\n", $signature);
				$signature = trim((string)$tmparr[1]);
				unset($tmparr);
				// decode signature
	...
	public function Curve($x0, $y0, $x1, $y1, $x2, $y2, $x3, $y3, $style='', $line_style=array(), $fill_color=array()) {
		...
		if (!(false === strpos((string)$style, 'F')) AND is_array($fill_color)) {
			$this->setFillColorArray($fill_color);
		}
		$op = TCPDF_STATIC::getPathPaintOperator($style);
		if ($line_style) {
	...
	protected function _putbookmarks() {
		...
		foreach ($this->outlines as $i => $o) {
			...
			$title = preg_replace((string)$nltags, "\n", $o['t']);
			...
			if (isset($o['u']) AND !empty($o['u'])) {
				...
				if (is_string($o['u'])) {
					if ($o['u'][0] == '#') {
						// internal destination
						$out .= ' /Dest /'.TCPDF_STATIC::encodeNameObject(substr((string)$o['u'], 1));
					} elseif ($o['u'][0] == '%') {
						// embedded PDF file
						$filename = basename(substr((string)$o['u'], 1));
	...
	public function ListBox($name, $w, $h, $values, $prop=array(), $opt=array(), $x=null, $y=null, $js=false) {
		...
		if ($js) {
			...
			$this->javascript .= 'f'.$name.'.setItems('.substr((string)$s, 1).');'."\n";
			return;
		}
		// get default style
		$prop = array_merge($this->getFormDefaultProp(), $prop);
	...
	public function colorRegistrationBar($x, $y, $w, $h, $transition=true, $vertical=false, $colors='A,R,G,B,C,M,Y,K') {
		if (strpos((string)$colors, 'ALLSPOT') !== false) {
			...
			if (!empty($spot_colors)) {
				$spot_colors = substr((string)$spot_colors, 1);
	...
	public function CoonsPatchMesh($x, $y, $w, $h, $col1=array(), $col2=array(), $col3=array(), $col4=array(), $coords=array(0.00,0.0,0.33,0.00,0.67,0.00,1.00,0.00,1.00,0.33,1.00,0.67,1.00,1.00,0.67,1.00,0.33,1.00,0.00,1.00,0.00,0.67,0.00,0.33), $coords_min=0, $coords_max=1, $antialias=false) {
		...
		for ($i=0; $i < $count_patch; ++$i) {
			$this->gradients[$n]['stream'] .= chr((string)$patch_array[$i]['f']); //start with the edge flag as 8 bit
			...
			for ($j=0; $j < $count_cols; ++$j) {
				...
				$this->gradients[$n]['stream'] .= chr((string)$patch_array[$i]['colors'][$j]['r']);
				$this->gradients[$n]['stream'] .= chr((string)$patch_array[$i]['colors'][$j]['g']);
				$this->gradients[$n]['stream'] .= chr((string)$patch_array[$i]['colors'][$j]['b']);
			}
		}
		//paint the gradient
		$this->_out('/Sh'.$n.' sh');
	...
	function _putshaders() {
		...
		foreach ($this->gradients as $id => $grad) {
			if (($grad['type'] == 2) OR ($grad['type'] == 3)) {
				...
				$out .= ' /Functions ['.trim((string)$functions).']';
				$out .= ' /Bounds ['.trim((string)$bounds).']';
				$out .= ' /Encode ['.trim((string)$encode).']';
				$out .= ' >>';
				$out .= "\n".'endobj';
				$this->_out($out);
				for ($i = 1; $i < $num_cols; ++$i) {
	...
	public function ImageEps($file, $x=null, $y=null, $w=0, $h=0, $link='', $useBoundingBox=true, $align='', $palign='', $border=0, $fitonpage=false, $fixoutvals=false) {
		...
		if (count($regs) > 1) {
			$version_str = trim((string)$regs[1]); # e.g. "Adobe Illustrator(R) 8.0"
		...
		if (count($regs) > 1) {
			list($x1, $y1, $x2, $y2) = explode(' ', trim((string)$regs[1]));
		} else {
		...
		for ($i=0; $i < $cnt; ++$i) {
			...
			$color_name = '';
			if (strcasecmp('x', substr(trim((string)$line), -1)) == 0) {
				if (preg_match('/\([^\)]*\)/', $line, $matches) > 0) {
					...
					$color_name = substr((string)$color_name, 1, -1);
				}
			}
			$chunks = explode(' ', $line);
			...
			if (($cmd == 'Xa') OR ($cmd == 'XA')) {
				...
				$g = array_pop($chunks);
				$r = array_pop($chunks);
				$this->_out(''.$r.' '.$g.' '.$b.' '.($cmd=='Xa'?'rg':'RG')); //substr((string)$line, 0, -2).'rg' -> in EPS (AI8): c m y k r g b rg!
				continue;
			}
			$skip = false;
			if ($fixoutvals) {
			...
			switch ($cmd) {
				...
				case 'C': {
					...
					$line[($len - 1)] = strtolower((string)$cmd);
					$this->_out($line);
					break;
				}
				case 'b':
	...
	protected function getHtmlDomArray($html) {
				...
				if (preg_match('/type[\s]*=[\s]*"text\/css"/', $link, $type)) {
					...
					if (empty($type) OR (isset($type[1]) AND (($type[1] == 'all') OR ($type[1] == 'print')))) {
						...
						if (preg_match('/href[\s]*=[\s]*"([^"]*)"/', $link, $type) > 0) {
							// read CSS data file
                            $cssdata = $this->getCachedFileContents(trim((string)$type[1]));
		...
		$csstagarray = '<cssarray>'.htmlentities(json_encode($css), ENT_QUOTES, 'UTF-8').'</cssarray>';
		...
		while (($offset < strlen((string)$html)) AND ($pos = strpos((string)$html, '</pre>', $offset)) !== false) {
			...
			$html_b = substr((string)$html, $offset, ($pos - $offset + 6));
		...
		while (($offset < strlen((string)$html)) AND ($pos = strpos((string)$html, '</textarea>', $offset)) !== false) {
			...
			$html = $html_a.$html_b.substr((string)$html, $pos + 11);
			$offset = strlen((string)$html_a.$html_b);
		}
		...
		$tagpattern = '/(<[^>]+>)/';
		// explodes the string
		$a = preg_split((string)$tagpattern, $html, -1, PREG_SPLIT_DELIM_CAPTURE | PREG_SPLIT_NO_EMPTY);
		// count elements
		$maxel = count($a);
		$elkey = 0;
		...
		while ($elkey < $maxel) {
			...
			$element = $a[$elkey];
			$dom[$key]['elkey'] = $elkey;
			if (preg_match((string)$tagpattern, $element)) {
				// html tag
				$element = substr((string)$element, 1, -1);
				// get tag name
				preg_match('/[\/]?([a-zA-Z0-9]*)/', $element, $tag);
				$tagname = strtolower((string)$tag[1]);
				...
				if ($element[0] == '/') {
					...
					$dom[$key]['parent'] = end($level);
					if ((substr((string)$element, -1, 1) == '/') OR (in_array($dom[$key]['value'], $selfclosingtags))) {
						// self-closing tag
						$dom[$key]['self'] = true;
					} else {
						// opening tag
                    ...
                    foreach($attr_array[1] as $id => $name) {
                        $dom[$key]['attribute'][strtolower((string)$name)] = $attr_array[2][$id];
                    }
					if (!empty($css)) {
					...
					if (isset($dom[$key]['attribute']['style']) AND !empty($dom[$key]['attribute']['style'])) {
                        ...
                        foreach($style_array[1] as $id => $name) {
                            ...
                            $dom[$key]['style'][strtolower((string)$name)] = trim((string)$style_array[2][$id]);
                        }
				...
				if (!empty($dom[$dom[$key]['parent']]['text-transform'])) {
					...
					if (function_exists('mb_convert_case')) {
						...
						switch ($dom[$dom[$key]['parent']]['text-transform']) {
							case 'capitalize': {
								$element = ucwords(strtolower((string)$element));
								break;
							}
							case 'uppercase': {
	...
	public function serializeTCPDFtag($method, $params=array()) {
		...
		return strlen((string)$hash).'+'.$hash.'+'.$encoded;
	}
	...
	protected function unserializeTCPDFtag($data) {
		$hpos = strpos((string)$data, '+');
		$hlen = intval(substr((string)$data, 0, $hpos));
		$hash = substr((string)$data, $hpos + 1, $hlen);
		$encoded = substr((string)$data, $hpos + 2 + $hlen);
				...
				if ((!$this->newline) AND ($dom[$key]['value'] == 'img') AND (isset($dom[$key]['height'])) AND ($dom[$key]['height'] > 0)) {
					...
					if (!$autolinebreak) {
						...
						if ($this->page > $startlinepage) {
							...
							$linebeg = substr((string)$pagebuff, $startlinepos, ($curpos - $startlinepos));
							$tstart = substr((string)$pagebuff, 0, $startlinepos);
							$tend = substr((string)$this->getPageBuffer($startlinepage), $curpos);
							// remove line from previous page
							$this->setPageBuffer($startlinepage, $tstart.''.$tend);
							$pagebuff = $this->getPageBuffer($this->page);
							$tstart = substr((string)$pagebuff, 0, $this->cntmrk[$this->page]);
							$tend = substr((string)$pagebuff, $this->cntmrk[$this->page]);
			...
			if ($this->newline AND (strlen((string)$dom[$key]['value']) > 0) AND ($dom[$key]['value'] != 'td') AND ($dom[$key]['value'] != 'th')) {
				...
				if (isset($startlinex)) {
					...
					if ($this->inxobj) {
						...
						$pstart = substr((string)$this->getPageBuffer($startlinepage), 0, $startlinepos);
						...
						if ($midpos > 0) {
							$pmid = substr((string)$this->getPageBuffer($startlinepage), $startlinepos, ($midpos - $startlinepos));
							$pend = substr((string)$this->getPageBuffer($startlinepage), $midpos);
						} else {
							$pmid = substr((string)$this->getPageBuffer($startlinepage), $startlinepos);
							$pend = '';
						}
					}
					if ((((($plalign == 'C') OR ($plalign == 'J') OR (($plalign == 'R') AND (!$this->rtl)) OR (($plalign == 'L') AND ($this->rtl)))))) {
						...
						if ($this->isRTLTextDir()) { // RTL
							...
							if ($pos1 > 0) {
								...
								if ($pos1 == $pos2) {
									$pmid = substr((string)$pmid, 0, ($pos1 + 2)).substr((string)$pmid, ($pos1 + 2 + $spacelen));
									if (substr((string)$pmid, $pos1, 4) == '[()]') {
							...
							if ($pos1 > 0) {
								...
								if ($pos1 == $pos2) {
									$pmid = substr((string)$pmid, 0, ($pos1 - $spacelen)).substr((string)$pmid, $pos1);
									$linew -= $one_space_width;
								}
							}
						}
								...
								while (preg_match('/([0-9\.\+\-]*)[\s](Td|cm|m|l|c|re)[\s]/x', $pmid, $strpiece, PREG_OFFSET_CAPTURE, $offset) == 1) {
									...
									if ($epsposend !== null) {
										...
										if (($epsposbeg > 0) AND ($epsposend > 0) AND ($offset > $epsposbeg) AND ($offset < $epsposend)) {
											...
											$pmid_b = substr((string)$pmid, 0, $epsposbeg);
											$pmid_m = substr((string)$pmid, $epsposbeg, ($epsposend - $epsposbeg));
											$pmid_e = substr((string)$pmid, $epsposend);
											$pmid = $pmid_b."\nq\n".$trx."\n".$pmid_m."\nQ\n".$pmid_e;
											$offset = $epsposend;
											continue;
										}
									...
									switch ($strpiece[2][0]) {
										...
										case 'l': {
											...
											if (preg_match('/([0-9\.\+\-]*)[\s]('.$strpiece[1][0].')[\s]('.$strpiece[2][0].')([\s]*)/x', $pmid, $pmatch) == 1) {
												...
												$pmid = str_replace((string)$pmatch[0], $newpmid, $pmid);
	...
	protected function hasExtForbiddenProtocol($path) {
		return ((strpos((string)$path, '://') !== false)
		...
		switch($tag['value']) {
			...
			case 'img': {
				...
				if ($imgsrc[0] === '@') {
					...
					break;
				} elseif ( $this->allowLocalFiles && substr((string)$imgsrc, 0, 7) === 'file://') {
				...
				if (isset($this->HREF['url']) AND !TCPDF_STATIC::empty_string($this->HREF['url'])) {
					...
					if ($imglink[0] == '#' AND isset($imglink[1]) AND is_numeric($imglink[1])) {
						...
						if (isset($lnkdata[0])) {
							$page = intval(substr((string)$lnkdata[0], 1));
				...
				if (!$in_table_head) { // we are not inside a thead section
					...
					if (($this->page == ($this->numpages - 1)) AND ($this->pageopen[$this->numpages])) {
						...
						if (($plendiff > 0) AND ($plendiff < 60)) {
							$pagediff = substr((string)$this->getPageBuffer($this->numpages), $this->emptypagemrk[$this->numpages], $plendiff);
							if (substr((string)$pagediff, 0, 5) == 'BT /F') {
	...
	public function setLIsymbol($symbol='!') {
		...
		if (substr((string)$symbol, 0, 4) == 'img|') {
	...
	protected function putHtmlListBullet($listdepth, $listtype='', $size=10) {
		...
		if ($listtype == '^') {
			...
			$listtype = 'decimal';
		} elseif (substr((string)$listtype, 0, 4) == 'img|') {
		...
		switch ($listtype) {
			...
			case 'img': {
				...
				$imgtype = strtolower((string)$img[1]);
				$prev_y = $this->y;
				switch ($imgtype) {
					case 'svg': {
	...
	public function movePage($frompage, $topage) {
		...
		if (preg_match_all('/this\.addField\(\'([^\']*)\',\'([^\']*)\',([0-9]+)/', $this->javascript, $pamatch) > 0) {
			foreach($pamatch[0] as $pk => $pmatch) {
				...
				$this->javascript = str_replace((string)$pmatch, $newjs, $this->javascript);
			}
			unset($pamatch);
		}
		// return to last page
			...
			for ($p = $page_first; $p <= $page_last; ++$p) {
				...
				for ($n = 1; $n <= $maxpage; ++$n) {
					...
					foreach ($pnalias['u'] as $u) {
						$sfill = str_repeat((string)$filler, max(0, (strlen((string)$u) - strlen((string)$nu.' '))));
						if ($this->rtl) {
						...
						$temppage = str_replace((string)$u, $nr, $temppage);
					}
					foreach ($pnalias['a'] as $a) {
						$sfill = str_repeat((string)$filler, max(0, (strlen((string)$a) - strlen((string)$na.' '))));
						if ($this->rtl) {
							$nr = $na.' '.$sfill;
						} else {
							$nr = $sfill.' '.$na;
						}
						$temppage = str_replace((string)$a, $nr, $temppage);
					}
				}
				// save changes
		...
		if (!TCPDF_STATIC::empty_string($page)) {
			for ($p = $page_first; $p <= $page_last; ++$p) {
				...
				for ($n = 1; $n <= $maxpage; ++$n) {
					...
					foreach ($pnalias['u'] as $u) {
						if ($correct_align) {
							$sfill = str_repeat((string)$filler, (strlen((string)$u) - strlen((string)$nu.' ')));
							if ($this->rtl) {
								$nr = $nu.TCPDF_FONTS::UTF8ToUTF16BE(' '.$sfill, false, $this->isunicode, $this->CurrentFont);
							} else {
								$nr = TCPDF_FONTS::UTF8ToUTF16BE($sfill.' ', false, $this->isunicode, $this->CurrentFont).$nu;
					...
					foreach ($pnalias['a'] as $a) {
						if ($correct_align) {
							$sfill = str_repeat((string)$filler, (strlen((string)$a) - strlen((string)$na.' ')));
							if ($this->rtl) {
								$nr = $na.' '.$sfill;
							} else {
								$nr = $sfill.' '.$na;
	...
	*/
	...
	protected function hyphenateWord($word, $patterns, $dictionary=array(), $leftmin=1, $rightmin=2, $charmin=1, $charmax=8) {
		...
		if (preg_match((string)$pattern, $word_string) > 0) {
			// email
			return $word;
		}
		...
		foreach ($unichars as $char) {
			if ((!$intag) AND (!$skip) AND TCPDF_FONT_DATA::$uni_type[$char] == 'L') {
				...
				if (chr((string)$char) == '<') {
					...
					$intag = true;
				} elseif ($intag AND (chr((string)$char) == '>')) {
					// end of HTML tag
					$intag = false;
					// check for style tag
	...
	public function getFontFamilyName($fontfamily) {
		...
		foreach ($fontslist as $font) {
			...
			$pattern[] = '/^monospace/';
			$replacement[] = 'courier';
			$font = preg_replace((string)$pattern, $replacement, $font);
	...
	public function ImageSVG($file, $x=null, $y=null, $w=0, $h=0, $link='', $align='', $palign='', $border=0, $fitonpage=false) {
		...
		if ($aspect_ratio_align != 'none') {
			...
			// correct X alignment
			switch (substr((string)$aspect_ratio_align, 1, 3)) {
				case 'Min': {
					// do nothing
					break;
				}
			...
			// correct Y alignment
			switch (substr((string)$aspect_ratio_align, 5)) {
				case 'Min': {
					// do nothing
					break;
				}
	...
	protected function SVGPath($d, $style='') {
		...
		foreach ($paths as $key => $val) {
			// get curve type
			$cmd = trim((string)$val[1]);
			if (strtolower((string)$cmd) == $cmd) {
			...
			if (isset($val[2])) {
				...
				$rawparams = preg_split('/([\,\s]+)/si', trim((string)$val[2]));
					...
					foreach ($params as $ck => $cp) {
						...
						if ((($ck + 1) % 7) == 0) {
							...
							if ((abs($x0 - $x) < $minlen) AND (abs($y0 - $y) < $minlen)) {
								...
								$pie = false;
								if (($key == 0) AND (isset($paths[($key + 1)][1])) AND (trim((string)$paths[($key + 1)][1]) == 'z')) {
									$pie = true;
								}
			...
			case 'image': {
				...
				if (!$clipping) {
					...
					if (preg_match('/^data:image\/[^;]+;base64,/', $img, $m) > 0) {
						...
						$img = '@'.base64_decode(substr((string)$img, strlen((string)$m[0])));
					} else {
						// fix image path
	...
	{
		if ((strpos((string)$file, '://') !== false) && ((substr((string)$file, 0, 7) !== 'file://') || (!$this->allowLocalFiles))) {
			// forbidden protocol
			return false;
		}
		return @unlink($file);
