...
class TCPDFBarcode {
	...
	protected function barcode_c128($code, $type='') {
		...
		switch(strtoupper((string)$type)) {
			case 'A': { // MODE A
				...
				for ($i = 0; $i < $len; ++$i) {
					$char = $code[$i];
					$char_id = ord((string)$char);
					if (($char_id >= 241) AND ($char_id <= 244)) {
						$code_data[] = $fnc_a[$char_id];
					} elseif (($char_id >= 0) AND ($char_id <= 95)) {
						$code_data[] = strpos((string)$keys_a, $char);
					} else {
						return false;
					}
				}
			...
			case 'B': { // MODE B
				...
				for ($i = 0; $i < $len; ++$i) {
					...
					if (($char_id >= 241) AND ($char_id <= 244)) {
						...
						$code_data[] = strpos((string)$keys_b, $char);
					} else {
						return false;
					}
				}
				...
				if (isset($numseq[1]) AND !empty($numseq[1])) {
					...
					foreach ($numseq[1] as $val) {
						...
						if ($offset > $end_offset) {
							...
							$sequence = array_merge($sequence, $this->get128ABsequence(substr((string)$code, $end_offset, ($offset - $end_offset))));
						}
						// numeric sequence
						...
						$sequence[] = array('C', substr((string)$code, $offset, $slen), $slen);
						$end_offset = $offset + $slen;
					}
					if ($end_offset < $len) {
						$sequence = array_merge($sequence, $this->get128ABsequence(substr((string)$code, $end_offset)));
					}
				} else {
	...
	protected function get128ABsequence($code) {
		...
		if (isset($numseq[1]) AND !empty($numseq[1])) {
			...
			foreach ($numseq[1] as $val) {
				...
				if ($offset > $end_offset) {
					// B sequence
					$sequence[] = array('B', substr((string)$code, $end_offset, ($offset - $end_offset)), ($offset - $end_offset));
				}
				// A sequence
				$slen = strlen((string)$val[0]);
				$sequence[] = array('A', substr((string)$code, $offset, $slen), $slen);
				$end_offset = $offset + $slen;
			}
			if ($end_offset < $len) {
				$sequence[] = array('B', substr((string)$code, $end_offset), ($len - $end_offset));
			}
		} else {
			// only B sequence
	...
	protected function barcode_eanupc($code, $len=13) {
		...
		$data_len = $len - 1;
		//Padding
		$code = str_pad((string)$code, $data_len, '0', STR_PAD_LEFT);
	...
	protected function barcode_eanext($code, $len=5) {
		//Padding
		$code = str_pad((string)$code, $len, '0', STR_PAD_LEFT);
		// calculate check digit
		if ($len == 2) {
			$r = $code % 4;
		} elseif ($len == 5) {
	...
	protected function barcode_imb($code) {
		...
		// pad to get 13 bytes
		$binary_code = str_pad((string)$binary_code, 26, '0', STR_PAD_LEFT);
