...
// -----------------------------------------------------------------------------
	...
	public static function decodeFilterASCII85Decode($data) {
		...
		for ($i = 0; $i < $data_length; ++$i) {
			...
			if ($char == 122) { // 'z'
				...
				if ($group_pos == 4) {
					$decoded .= chr((string)$tuple >> 24).chr((string)$tuple >> 16).chr((string)$tuple >> 8).chr((string)$tuple);
					$tuple = 0;
					$group_pos = 0;
				} else {
					++$group_pos;
		...
		switch ($group_pos) {
			case 4: {
				$decoded .= chr((string)$tuple >> 24).chr((string)$tuple >> 16).chr((string)$tuple >> 8);
				break;
			}
			case 3: {
				$decoded .= chr((string)$tuple >> 24).chr((string)$tuple >> 16);
				break;
			}
			case 2: {
				$decoded .= chr((string)$tuple >> 24);
				break;
			}
			case 1: {
				self::Error('decodeFilterASCII85Decode: invalid code');
	...
	public static function decodeFilterLZWDecode($data) {
		...
		$bitstring = '';
		for ($i = 0; $i < $data_length; ++$i) {
			$bitstring .= sprintf('%08b', ord((string)$data{$i}));
		}
		...
		$dictionary = array();
		for ($i = 0; $i < 256; ++$i) {
			$dictionary[$i] = chr((string)$i);
		}
		// previous val
		$prev_index = 0;
		// while we encounter EOD marker (257), read code_length bits
		while (($data_length > 0) AND (($index = bindec(substr((string)$bitstring, 0, $bitlen))) != 257)) {
			// remove read bits from string
			$bitstring = substr((string)$bitstring, $bitlen);
	...
	public static function decodeFilterRunLengthDecode($data) {
		...
		while($i < $data_length) {
			...
			if ($byte == 128) {
				...
				$decoded .= substr((string)$data, ($i + 1), ($byte + 1));
				// move to next block
				$i += ($byte + 2);
			} else {
				// if length is in the range 129 to 255,
				// the following single byte shall be copied 257 - length (2 to 128) times during decompression
				$decoded .= str_repeat((string)$data{($i + 1)}, (257 - $byte));
				// move to next block
				$i += 2;
			}
		}
