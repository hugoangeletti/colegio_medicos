...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	...
	public static function getImageFileType($imgfile, $iminfo=array()) {
		...
		if (empty($type)) {
			...
			if (isset($fileinfo['extension']) AND (!TCPDF_STATIC::empty_string($fileinfo['extension']))) {
				$type = strtolower(trim((string)$fileinfo['extension']));
			}
		}
		if ($type == 'jpg') {
			$type = 'jpeg';
	...
	public static function _parsejpeg($file) {
		...
		while (($pos = strpos((string)$data, "ICC_PROFILE\0", $offset)) !== false) {
			...
			$msn = max(1, ord((string)$data[($pos + 12)]));
			// number of markers (total of APP2 used)
			$nom = max(1, ord((string)$data[($pos + 13)]));
			// get sequence segment
			$icc[($msn - 1)] = substr((string)$data, ($pos + 14), $length);
		...
		if (count($icc) > 0) {
			ksort($icc);
			$icc = implode('', $icc);
			if ((ord((string)$icc[36]) != 0x61) OR (ord((string)$icc[37]) != 0x63) OR (ord((string)$icc[38]) != 0x73) OR (ord((string)$icc[39]) != 0x70)) {
				// invalid ICC profile
				$icc = false;
			}
		} else {
	...
	public static function _parsepng($file) {
		...
		do {
			...
			if ($type == 'PLTE') {
				...
				if ($ct == 0) { // DeviceGray
					$trns = array(ord((string)$t[1]));
				} elseif ($ct == 2) { // DeviceRGB
					$trns = array(ord((string)$t[1]), ord((string)$t[3]), ord((string)$t[5]));
				} else { // Indexed
					if ($n > 0) {
						$trns = array();
						for ($i = 0; $i < $n; ++ $i) {
