...
class TCPDF {
	...
	protected function adjustCellPadding($brd=0) {
		...
		foreach ($brd as $border => $style) {
			...
			if ((strpos((string)$border,'T') !== false) AND ($this->cell_padding['T'] < $adj)) {
				$this->cell_padding['T'] = $adj;
			}
			if ((strpos((string)$border,'R') !== false) AND ($this->cell_padding['R'] < $adj)) {
				$this->cell_padding['R'] = $adj;
			}
			if ((strpos((string)$border,'B') !== false) AND ($this->cell_padding['B'] < $adj)) {
				$this->cell_padding['B'] = $adj;
			}
			if ((strpos((string)$border,'L') !== false) AND ($this->cell_padding['L'] < $adj)) {
						...
						case 'link': {
							if (is_string($pl['txt'])) {
								if ($pl['txt'][0] == '#') {
									...
									$annots .= ' /Dest /'.TCPDF_STATIC::encodeNameObject(substr((string)$pl['txt'], 1));
								} elseif ($pl['txt'][0] == '%') {
									// embedded PDF file
									...
									if (empty($parsedUrl['scheme']) AND (strtolower(substr((string)$parsedUrl['path'], -4)) == '.pdf')) {
				...
				if (preg_match('/type[\s]*=[\s]*"text\/css"/', $link, $type)) {
					...
					if (empty($type) OR (isset($type[1]) AND (($type[1] == 'all') OR ($type[1] == 'print')))) {
						...
						if (preg_match('/href[\s]*=[\s]*"([^"]*)"/', $link, $type) > 0) {
							// read CSS data file
							$cssdata = TCPDF_STATIC::fileGetContents(trim((string)$type[1]));
			...
			case 'img': {
				if (!empty($tag['attribute']['src'])) {
					if ($tag['attribute']['src'][0] === '@') {
						// data stream
						$tag['attribute']['src'] = '@'.base64_decode(substr((string)$tag['attribute']['src'], 1));
						$type = '';
					} else {
						// get image type
		...
		switch($name) {
			...
			case 'stop': {
				...
				if (substr((string)$attribs['offset'], -1) == '%') {
					$offset = floatval(substr((string)$attribs['offset'], -1)) / 100;
				} else {
