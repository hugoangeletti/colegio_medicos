...
// -----------------------------------------------------------------------------
	...
	public function __construct($data, $cfg=array()) {
		...
		$this->pdfdata = substr((string)$data, $trimpos);
		// get length
	...
	protected function getRawObject($offset=0) {
		...
		switch ($char) {
			...
			case '/': { // \x2F SOLIDUS
				...
				$objtype = $char;
				++$offset;
				if (preg_match('/^([^\x00\x09\x0a\x0c\x0d\x20\s\x28\x29\x3c\x3e\x5b\x5d\x7b\x7d\x2f\x25]+)/', substr((string)$this->pdfdata, $offset, 256), $matches) == 1) {
			...
			case ')': { // \x29 RIGHT PARENTHESIS
				...
				if ($char == '(') {
					...
					$objval = substr((string)$this->pdfdata, $offset, ($strpos - $offset - 1));
					$offset = $strpos;
				}
				break;
			}
			...
			case '>': { // \x3E GREATER-THAN SIGN
				if (isset($this->pdfdata{($offset + 1)}) AND ($this->pdfdata{($offset + 1)} == $char)) {
					...
					$objtype = $char;
					++$offset;
					if (($char == '<') AND (preg_match('/^([0-9A-Fa-f\x09\x0a\x0c\x0d\x20]+)>/iU', substr((string)$this->pdfdata, $offset), $matches) == 1)) {
			...
			default: {
				if (substr((string)$this->pdfdata, $offset, 6) == 'endobj') {
					// indirect object
					$objtype = 'endobj';
					$offset += 6;
				} elseif (substr((string)$this->pdfdata, $offset, 4) == 'null') {
					// null object
					$objtype = 'null';
					$offset += 4;
					$objval = 'null';
				} elseif (substr((string)$this->pdfdata, $offset, 4) == 'true') {
					// boolean true object
					$objtype = 'boolean';
					$offset += 4;
					$objval = 'true';
				} elseif (substr((string)$this->pdfdata, $offset, 5) == 'false') {
					// boolean false object
					$objtype = 'boolean';
					$offset += 5;
					$objval = 'false';
				} elseif (substr((string)$this->pdfdata, $offset, 6) == 'stream') {
					// start stream object
					$objtype = 'stream';
					$offset += 6;
					if (preg_match('/^([\r]?[\n])/isU', substr((string)$this->pdfdata, $offset), $matches) == 1) {
						$offset += strlen((string)$matches[0]);
						if (preg_match('/(endstream)[\x09\x0a\x0c\x0d\x20]/isU', substr((string)$this->pdfdata, $offset), $matches, PREG_OFFSET_CAPTURE) == 1) {
							$objval = substr((string)$this->pdfdata, $offset, $matches[0][1]);
							$offset += $matches[1][1];
						}
					}
				} elseif (substr((string)$this->pdfdata, $offset, 9) == 'endstream') {
					// end stream object
					$objtype = 'endstream';
					$offset += 9;
				} elseif (preg_match('/^([0-9]+)[\s]+([0-9]+)[\s]+R/iU', substr((string)$this->pdfdata, $offset, 33), $matches) == 1) {
					// indirect object reference
					$objtype = 'objref';
					$offset += strlen((string)$matches[0]);
					$objval = intval($matches[1]).'_'.intval($matches[2]);
				} elseif (preg_match('/^([0-9]+)[\s]+([0-9]+)[\s]+obj/iU', substr((string)$this->pdfdata, $offset, 33), $matches) == 1) {
					// object start
					$objtype = 'obj';
					$objval = intval($matches[1]).'_'.intval($matches[2]);
					$offset += strlen((string)$matches[0]);
					...
					// numeric object
					$objtype = 'numeric';
					$objval = substr((string)$this->pdfdata, $offset, $numlen);
					$offset += $numlen;
				}
				break;
			}
	...
	protected function getIndirectObject($obj_ref, $offset=0, $decoding=true) {
		...
		if (strpos((string)$this->pdfdata, $objref, $offset) != $offset) {
			...
			if ($v[0] == '/') {
				if (($v[1] == 'Length') AND (isset($sdic[($k + 1)])) AND ($sdic[($k + 1)][0] == 'numeric')) {
					...
					if ($declength < $slength) {
						$stream = substr((string)$stream, 0, $declength);
						$slength = $declength;
					}
